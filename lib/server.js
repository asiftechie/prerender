// ----------------------------------------------------------------------------
// Thanks to prerender team for putting it together
// 
// Port of prerender into my repository to experiment for a client
// So far the difference of this port is to run server in a different port
// 
// Asif Habib (asif.techie@gmail.com)
// Technical Consultant, Entrepreneur, Developer, Architect
// Mar 22, 2014
// 
// How to use: 
// 1) change index.html (or similar) and add header -- <meta name="fragment" content="!">  
// 2) Run prerender first on port 3000
// 3) Run your app on port 8080
// 4) Assuming your URL: http://localhost:8080/shop/blabla
// 5) Try in browser: http://localhost:3000/shop/blabla?_escaped_fragment_=
// 
// ----------------------------------------------------------------------------
...
var MY_SERVER_PORT = 'http://localhost:8080/';  // if my app is in different port from prerender node
...
server.onPhantomPageCreate = function(req, res) {
    ...
    req.prerender.page.get('settings.userAgent', function(userAgent) {
        ...
        _this._pluginEvent("onPhantomPageCreate", [_this.phantom, req, res], function() {
            ...
            req.prerender.page.open(MY_SERVER_PORT + encodeURI(req.prerender.url.replace(/%20/g, ' ')), function(status) {
...
server.onResourceRequestedCallback = function (req, res, requestData) {
...
server.onResourceReceived = function (req, res, response) {
    ...
    if(normalizeUrl(req.prerender.url) === normalizeUrl(response.url) && response.status >= 300 && response.status <= 399) {
    ...
    if ('end' === response.stage) { 
        ...
        if (normalizeUrl(req.prerender.url) === normalizeUrl(response.url)) {
            req.prerender.pendingRequests--;
...
// fetching a resource
server.onResourceError = function(req, res, resourceError) {
    req.prerender.pendingRequests--;
};
...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    ...
    if(req.prerender.stage < 1 && (force || (req.prerender.pendingRequests <= 0 && timeSinceLastRequest > (this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST)) || timedOut)) {
        ...
        clearInterval(req.prerender.downloadTimeout);
        req.prerender.downloadTimeout = null;
...
// 
// A utility function to print object in run/real time 
// 
function printObject(name, prefix, o, noOfRows) {
  var out = prefix + ' ' + name + ' ' ;
  console.log('--------------------------------::' + name + '.begin.' + noOfRows);
  var printCount = 1;
  for (var p in o) {
    if (printCount > noOfRows)  
    {
        console.log('--------------------------------::' + name + '.' + printCount );
        break;
    }
    else
        out += ' :: '; 
...
server._send = function(req, res, statusCode, options) {
    ...
    this._pluginEvent("beforeSend", [req, res], function() {
        ...
        // comment if you don't want to see details. print only 50 items within object
        // too noisy -- printObject('req.prerender', 'req.prerender', req.prerender, 50);
        res.end();
